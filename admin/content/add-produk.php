...
if (isset($_POST['submit'])) {
  ...
  if (isset($_POST['edit'])) {
    ...
    mysqli_query($config, "UPDATE produk 
        SET nama_produk='$nama_produk', kode_produk='$kode_produk',  deskripsi='$deskripsi', harga_jual='$harga_jual', 
            kategori_id='$kategori_id', stok='$stok', gambar='$gambar' 
        WHERE id='$idEdit'");
  ...
  // Simpan ke database
  $queryAdd = mysqli_query($config, "INSERT INTO produk (nama_produk, kode_produk, deskripsi, harga_jual, kategori_id, stok, gambar) VALUES ('$nama_produk', '$no_transaction','$deskripsi', '$harga_jual', '$kategori_id', '$stok', '$gambar')");
    ...
    <div class="card-body">
      <form method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group mb-3">
          ...
          <select name="stok" class="form-control select2" id="simple-select2" required>
            <optgroup label="Pilih Stok">
              <option value="0" <?= (isset($rowEdit['stok']) && $rowEdit['stok'] == 0) ? ' selected' : '' ?>>
                <?= getOrderStatus(0) ?>
              </option>
              <option value="1" <?= (isset($rowEdit['stok']) && $rowEdit['stok'] == 1) ? ' selected' : '' ?>>
                <?= getOrderStatus(1) ?>
              </option>
            </optgroup>
          </select>
